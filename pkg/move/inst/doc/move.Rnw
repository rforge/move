...
@
\end{center}
\caption{Plotting the track and adding the burst information as colored lines, points or circles (the color corresponds to the brustID; the size of the circles represents in three steps the relative amount of time per segment)}
\label{fig:burst}
\end{figure}
