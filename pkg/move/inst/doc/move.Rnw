...
The \fct{plot} function simply plots the different segments as colored lines. \\
...
\begin{figure}
\begin{center}
<<label=figburst, fig=TRUE, echo=TRUE, results=hide, height=4>>=
par(mfrow=c(1,2))
plot(leroy_b, lwd=2)
plotBursts(leroy_b, breaks=3, add=FALSE, pch=19)
@
\end{center}
\caption{Plotting the track and adding the burst information as colored lines or circles (the color corresponds to the brustID; the size of the circles represents in three steps the realtive amount of time per segment)}
\label{fig:burst}
\end{figure}
