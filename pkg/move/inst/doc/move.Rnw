...
Bursted tracks can be plotted with \fct{plotBursts}. The function plots different colored circles that indicate different categories of the track. The size of the circles is by default the relative time per segment compared to the track. It is possible to calculate the size with a different function using the \texttt{sizeFun} argument of the function. (Figure \ref{fig:burst})
