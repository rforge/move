...
An important function to work with objects is the \fct{show} funciton. Depending on the implementaiton it gives an overview of the object one is working with. The funciton can either called by the function name or just by entering the name of the object:
...
Both ways return the same information. This is for Move objects the class name, number of coordinates, the map extent, the coordinate reference system, number of additional variables, names of the additional variables and their minimum and maximum values, the sensor type
...
<<transformData>>=
proj4string(ricky)
ricky <- spTransform(x=ricky, CRSobj="+proj=aeqd", center=TRUE)  
proj4string(ricky)
@
