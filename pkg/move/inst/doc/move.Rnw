...
An important function to work with objects is the \fct{show} funciton. Depending on the implementaiton it gives an overview of the object one is working with. The funciton can either called by entering the name of the object directly, or by calling the function with the object:
...
\begin{figure}[ht]
\begin{center}
<<label=fig1, fig=TRUE, echo=FALSE, results=hide, height=4>>=
<<fig1plot>>
@
\end{center}
...
Bursted tracks can be plotted with \fct{plotBursts}. The function plots different colored circles that indicate different categories of the track. The size of the circles is by default the realtive time per segment compared to the track. It is possible to calculate the size with a different function using the \texttt{sizeFun} argument of the function. (Figure \ref{fig:burst})

\begin{figure}
\begin{center}
<<label=figburst, fig=TRUE, echo=TRUE, results=hide>>=
plotBursts(leroy_b, breaks=3, add=FALSE, pch=19)
@
\end{center}
\caption{Plotting the track and adding the burst information as colored circles. Here the circle size indicates the relative time per segment compared to the whole track and the color indicates different behaviors that were assigned before.}
\label{fig:burst}
\end{figure}
...
ricky_dbbmm <- brownian.bridge.dyn(r, dimSize=150, location.error=23, ext=.3, time.step=6000)
@
...
hrBootstrap(x=leroy, rep=10, unin='km', unout='km2')
@
\end{center}
