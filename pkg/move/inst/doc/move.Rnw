...
A series of functions allows to plot, summarize and analyze the movement data. Individual \class{Move} objects can be stacked to a MoveStack object which incldues a series of animals and the additional data that are associated with them. This allows to operate on many animals at the same time. \\
...
Files from Movebank (\url{www.movebank.org}) can be imported with \texttt{move(x="path")}, where "path" is character string wich locates the file on the harddrive. It may look like: \\ \\ \texttt{> data <- move(x="/User/Documents/test.csv")}. \\ \\
...
<<importNonMovebank>>=
data2 <- read.csv(system.file("extdata","yorel.csv",package="move"))
head(data2) ##see the structure of the data-frames first six rows
...
Entering the object name calls the \fct{show} function of \class{Move} objects. This function provides an overview of the object. It returns the object class (\class{Move}), the animal ID (Yorel), number of coordinates (100), the extent of the coordinates, the coordinates reference system (or projection, here longitude-latitude), the number of columns of the imported data.frame and the names of the columns of the dara.frame, as well es the first and last timestamp and the duration of the observation.The \fct{show} works in a similar way for all other object types.\\
...
The show function works also for the stack object. It combinds the information of all objects that are stored in the \class{MoveStack}. \\
...
The Move package includes a bunch of functions to access the Movebank database. A simple function to 
