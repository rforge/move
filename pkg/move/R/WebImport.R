...
setMethod(f="getMovebank", 
          ...
          definition = function(entity_type, login, ...){
            ...
            if (login@rcurl){
              ...
              if(grepl(pattern="The requested download may contain copyrighted material", x=web)) stop("You need a permission to access this data set. Go to movebank.org and accept the license terms when downloading the data set (you only have to do this once per data set).")
...
#print(url)            
#print(str(data))
              if(grepl(pattern="You.may.only.download.it.if.you.agree.with.the.terms", x=names(data)[1])) stop("You need a permission to access this data set. Go to movebank.org and accept the license terms when downloading the data set (you only have to do this once per data set).")
