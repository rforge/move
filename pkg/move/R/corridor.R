...
setMethod(f = "corridor",
          ...
          definition=function(x, speedProp, circProp, plot, ...){
            ...
            speed <- (segLength*1000)/time.lag(x, unit="secs") #meter/second
            speedQuant <- speed>=quantile.default(speed, probs=speedProp, na.rm=T)
            ...
            circVarQuant <- (circVar<=as.numeric(quantile.default(circVar[circVar!=0], probs=circProp, na.rm=T))) & circVar!=0
