setClass(Class = ".UDStack", contains = c("RasterStack"), representation = representation(method = "character"), 
    prototype = prototype(method = as.character()), validity = function(object) {
        if (!all(apply(values(object), MARGIN = 2, FUN = function(X) isTRUE(all.equal(sum(X), 
            1))))) 
            stop("One or more of the used rasters are not a UD (sum is not equal to 1)")
    })

setClass(Class = ".UD", contains = c("RasterLayer"), representation = representation(method = "character"), 
    prototype = prototype(method = as.character()), validity = function(object) {
...
setClass(Class = "DBBMMStack", contains = c(".UDStack"), representation = representation(DBMvar = "dBMvarianceStack", 
    ext = "numeric"), prototype = prototype(ext = as.numeric()), validity = function(object) {
...
setClass(Class = "DBBMM", contains = c(".UD"), representation = representation(DBMvar = "dBMvariance", 
...
setMethod(f = "brownian.bridge.dyn", signature = c(object = "dBMvariance", raster = "RasterLayer", 
    ...
    interest <- (c(object@interest, 0) + c(0, object@interest))[1:length(object@interest)] != 
        0
...
setMethod(f = ".extcalc", signature = c(obj = "SpatialPointsDataFrame", ext = "numeric"), 
    definition = function(obj, ext) {
        ...
        if (length(ext) == 4) {
            ...
            stop("The ext argument must be 1, 2 or 4 numbers")
        }
...
setGeneric("outerProbability", function(raster, border = 0.1) {
    standardGeneric("outerProbability")
})
# }

setMethod(f = "outerProbability", signature = c(raster = "RasterLayer"), definition = function(raster, 
    border) {
...
setMethod(f = "contour", signature = c(x = ".UD"), definition = function(x, ...) {
...
})  #bart kami do we want to skip maxpixels? if not than there would be no reason to have a different function name to rasterToContour
# }
setMethod(f = "raster2contour", signature = c(x = "RasterLayer"), definition = function(x, 
    ...) {
...
setGeneric("summary")
setMethod(f = "summary", signature = c(object = "DBBMM"), definition = function(object) {
